...
source("mkpe_projection_train.R")

#N_x x N_x matrix containing within-domain similarities between drugs
wdd <- sprintf("./%s_simmat_dc.txt", network)
#N_z x N_z matrix containing within-domain similarities between target proteins
...
for (f in 1:fold) {
  #N_x x N_z matrix containing interactions between drugs and proteins
  ...
  parameters <- data.frame(epsilon, iteration, lambda_c, lambda_x, lambda_z,
                           learn_sigma_e, R, seed, sigma_e)
  
  #perform training
  state[[f]] <- mkpe_projection_train(Y[fold_indices != f, ], K_x[fold_indices != f, fold_indices != f], K_z, 
                                      K_x_feature[fold_indices != f, fold_indices != f], K_z_feature, parameters)
}
